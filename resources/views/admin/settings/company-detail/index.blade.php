...
@section('content')
    ...
    <div class="container-fluid" id="container-wrapper">
            ...
            <div class="col-lg-12">
                <div class="card mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        ...
                        <a href="{{ route('companydetail.create') }}" class="btn btn-primary">Tambah Konten</a>
                    </div>
                    <div class="table-responsive p-3">
                        <table class="table align-items-center table-flush table-hover" id="companydetail-table"
                            style="width: 100%">
        ...
        <div id="modal-companydetail-wrapper"></div>
    </div>
    <!---Container Fluid-->
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            NProgress.start();
            let table = $('#companydetail-table').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                ajax: {
                    url: `{{ route('companydetail.index') }}`,
                },
                columns: [
                    {
                        data: 'company',
            ...
            $(document).on('click', function(e) {
                if ($(e.target).hasClass('btn-delete')) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: `{{ url('admin/setting/companydetail') }}/${id}`,
                                method: 'DELETE',
                                data: {
                                    id: id,
                                    _token: "{{ csrf_token() }}"
                ...
                if ($(e.target).hasClass('btn-show')) {
                    let id = $(e.target).data('id');
                    $("#modal-companydetail-wrapper").load(`{{ url('admin/setting/companydetail') }}/${id}`);
